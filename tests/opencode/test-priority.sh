...
# Verifies that skills are resolved with correct priority: project > personal > superartes
...
# 1. Create in superartes location (lowest priority)
mkdir -p "$HOME/.config/opencode/superartes/skills/priority-test"
cat > "$HOME/.config/opencode/superartes/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superartes version of priority test skill
---
# Priority Test Skill (Superartes Version)

This is the SUPERARTES version of the priority test skill.

PRIORITY_MARKER_SUPERARTES_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/superartes/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superartes version exists"
else
    echo "  [FAIL] Superartes version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides superartes
echo ""
echo "Test 2: Testing personal > superartes priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides superartes)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERARTES_VERSION"; then
    echo "  [FAIL] Superartes version loaded instead of personal"
    exit 1
else
...
# Test 3: Test that project overrides both personal and superartes
echo ""
echo "Test 3: Testing project > personal > superartes priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Superartes version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit superartes: prefix bypasses priority
echo ""
echo "Test 4: Testing superartes: prefix forces superartes version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load superartes:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERARTES_VERSION"; then
    echo "  [PASS] superartes: prefix correctly forces superartes version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] superartes: prefix did not force superartes version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
